...
class LoadRecommendedUseCase @Inject constructor(private val homeRepository: HomeRepository) {
    suspend fun execute(): Resource<List<SchoolResponse.SchoolData.DataSchool>> = homeRepository.getRecommended()

}

class LoadFeatureUseCase @Inject constructor(private val homeRepository: HomeRepository) {
    suspend fun execute(): Resource<List<SchoolResponse.SchoolData.DataSchool>> = homeRepository.getFeature()
}